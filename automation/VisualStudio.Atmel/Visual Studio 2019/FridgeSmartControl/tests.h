...
void loop() {
	...
	int_fast16_t temp = analogRead(SENSOR_TEMP0);
	...
	if (digitalRead(BUTTON_SET) || boolSetButton) {
		boolSetButton = true;

		// Set menu for more options
		/*Is on timer when boolSetButton was passed and user must pressing a button for a 5 seconds*/
		if (digitalRead(BUTTON_SET) && (millis() > (long)(startedWaiting + 5000UL) && !boolQuicklyChange) || boolSetMenu) {
			...
			if ((millis() > (long)(startedWaiting + 5000UL + 950UL)) && digitalRead(BUTTON_SET)) {
			...
			//progra-------------------------------------------------------
			if ((digitalRead(BUTTON_SET) || boolSetMenu)) {
				...
				if (meniuOptionSelectFun() == 1) { // P0 C-H
					...
					if (meniuOptionIsSelected) {
						...
						if (userP0 == 0)
							txtBigNumber(userP0, "-C-");
						else
							txtBigNumber(userP0, "-H-");
					}
					...
					// do EEPROM changes
					if (meniuOptionIsSelected) {
						txtBigNumber(float(userP1) / userFixP1, "C");
					}
		...
		// Quickly change temperature
		else if (digitalRead(BUTTON_SET) == LOW || boolQuicklyChange) {
			// set loop from boolean
			...
			// exit from tight loops!
			// event trigger if after some time will react to button set
			if ( /*Wait while user realesing a button*/(millis() > (long)(startedWaiting + 230UL)) && digitalRead(BUTTON_SET) || /*or timeout for 45s*/(millis() > (long)(startedWaiting + 45100UL))) {
				// reset whole set loop
			...
			if (boolQuicklyChange) {
				...
				display.println("-boolSetButton:" + String(boolSetButton));
				display.println("-boolQuicklyChange:" + String(boolQuicklyChange));
				display.println("boolSetMenu:" + String(boolSetMenu));
...
byte meniuOptionSelectFun() {
	...
	if ((digitalRead(BUTTON_UP) && meniuOptionSelected < meniuOptionsLenght) && meniuOptionIsPressing && !meniuOptionIsSelected) {
	...
	if ((digitalRead(BUTTON_DOWN) && meniuOptionSelected > 1) && meniuOptionIsPressing && !meniuOptionIsSelected) {
	...
	if (digitalRead(BUTTON_SET) && !meniuOptionIsSelected) {
...
void userSetValues() {
	...
	overwrited = writeMemory(MEMORY_P0, userP0); // for C-H Cooling Heating
	writeMemory(MEMORY_P1, userP1); // for temperature  logic range offset '0.5' range
	writeMemory(MEMORY_P2, userP2); // working temperature gap time
	writeMemory(MEMORY_P3, userP3); // over working timeout when working too long
	writeMemory(MEMORY_P4, userP4); // over working cooldown when working too long
}
...
void userChangeMeniuValue(byte* userPx, byte maxValue = 255, byte minValue = 0) {
	...
	// incrament
	if (digitalRead(BUTTON_UP) && *userPx < maxValue)
	{
		*userPx++;
		delay(100);
	}
	...
	// substract 
	if (digitalRead(BUTTON_DOWN) && *userPx > minValue)
	{
		*userPx--;
		delay(100);
	}
