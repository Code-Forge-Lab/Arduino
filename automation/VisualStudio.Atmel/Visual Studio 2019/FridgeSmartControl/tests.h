...
// display all stuff was updated
	...
	if (sensorTemp.temperature > -65)
	{
		...
		//oneMinuteFunction(); // timing requared values
		...
		{
			display.println("Next On:" + String(userP2Timeout));
		}
		...
		//sensor_25 > (24 + 2.0) = 25 > 27 
			...
			if (((sensorTemp.temperature > userP5FixTemperature() && !boolUserOffsetTemperatureRange)))
			{
				...
				boolOverworkState = true; // keep counting working time 

				if (/*overwork timeout*/!boolUserP3_Timeout && /*Frequent on timeout*/ userP2Timeout == 0) // if reached overwork state then turn off relay
				{
					boolUserP2Timeout = true; // enable counting wornking time and reset
					digitalWrite(RELAY_FRIDGE0, 255);
			...
			{
				// frequent on timeout
				if (boolUserP2Timeout)
				{
					boolUserP2Timeout = false;
					userP2Timeout = userP2; // set time for  timer; 
				}

				boolUserOffsetTemperatureRange = true; // set to offset mode
				boolOverworkState = true;
		...
		//sensor_25 > (24 - 2.0) = 25 < 24 
			...
			else
			{
				...
				if (boolUserP2Timeout)
				{
					boolUserP2Timeout = false; // disable counting wornking timeand 
					userP2Timeout = userP2; // set time for  timer; 
				}
...
void oneMinuteFunction() {
	...
	if (((long)clock_1min + 60000UL) < millis())
	{
		...
		if (userP2Timeout > 0)
			userP2Timeout--; // time  reduse in minute		
	}
