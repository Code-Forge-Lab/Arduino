...
void setup() {
	...
	initiate_updatePins();
	delay(500);
	attachInterrupt(0, MotorSpeedControlFuncZeroCrossDetected, FALLING); // attach interrupt function 
	delay(500);
...
void loop() {
	...
	motorSpeed = FANSPEEDRUN; // s
	motorPulseWidthTime = map(FANSPEEDRUN, 255, 0, 150, 8990); //7200 
