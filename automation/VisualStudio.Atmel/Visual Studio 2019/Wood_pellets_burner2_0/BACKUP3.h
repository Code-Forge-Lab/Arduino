// WOrking Basic Future of TRIAC Cotnrol
//#include <stdlib.h>
//#include <stddef.h>
...
void __FANMINSPEEDMIN() {
	...
	if (!isMotorSpeedEnabled)
		analogWrite(FANPIN, FANMINSPEED.getValue());
	else // else do a AC TRiac Single Pulse Control Until Zero Cross cycle ends
		motorPulseWidthTime = map(FANMINSPEED.getValue(), 255, 0, 150, 7200); //7200 
};
...
void funFAN() {
	...
	if (PELLETPUSHERMODE.getValue() == 1)
	{
		...
		printMenuFunc("Fanas " + Hot + "", &FANMAXSPEED, "Deg. " + RPM, &__FANMINSPEEDMAX);
...
void printstatus(bool print = false) {
	...
	case 2:
		...
		{
		case 1:
			lcd.print("Laikmatis " + RPM + String(FANSPEEDRUN));
	...
	case 4:
		...
		{
		case 1:
			lcd.print("1-" + workingWay);
			lcd.setCursor(0, 1);
			lcd.print("Pagal Laika");
			break;

		case 2:
			lcd.print("2-" + workingWay);
			lcd.setCursor(0, 1);
			lcd.print("Pagal Temp.");
			break;

		default:
			lcd.print(workingWay);
	...
	case 5:
		...
		{
		case 1:
			lcd.print("Busena:" + String(pelletSofRresult) + "," + String(pelletSoftCount));
			lcd.setCursor(0, 1);
			lcd.print("Pagal Laika");
			break;
   ...
   //     /analogWrite(PELLETPUSHERPIN, PELLETPUSHERMINSPEED.getValue()); //Give speed/power to motor Save
	...
	if (FANHOLDTIMEOUT_ON > 0) {
		...
		if (!isMotorSpeedEnabled)
			...
			motorPulseWidthTime = map(FANMAXSPEED.getValue(), 255, 0, 150, 7200); //7200 
	}
	else {
		// if normal DC motor then do PWM
		if (!isMotorSpeedEnabled)
			analogWrite(FANPIN, FANSPEEDRUN); // do Increse power per Time until reach a maximum set speed 
		
		else // else do a AC TRiac Single Pulse Control Until Zero Cross cycle ends
			motorPulseWidthTime = map(FANSPEEDRUN, 255, 0, 150, 7200); //7200
