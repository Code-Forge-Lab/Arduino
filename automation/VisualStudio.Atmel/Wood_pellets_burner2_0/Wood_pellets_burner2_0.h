...
bool FANFIRESTART = false; // set to burn slowly when pellet are droped on small fire to reduse smokde shock
float FANFIRESTARTTIMEOUT = 0; // timer 
...
void loop() {
	...
	if (FANFIRESTARTTIMEOUT > -1) { FANFIRESTARTTIMEOUT--; FANFIRESTART = false; };
            ...
            case 2 : // Temp.Low-Hight" // use temperature to shift between low or hight power condition
                 ...
                 if (Temperature.Temperature < TEMPMAX.getValue() ) {
					 ...
					 if (FANHOLDENABLE_ON == false) // if fan was started from LOW mode then try increase speed very slowly
					 {
						 ...
						 FANFIRESTARTTIMEOUT = 20 * OneSec;
						
						 sum = (FANMAXSPEED.getValue() - FANMINSPEED.getValue()) ;
					 }

					   if (FANFIRESTART)// if true to slow start fan
					   {    
						   
						   FANSPEEDRUN = (sum / FANFIRESTARTTIMEOUT) + FANMINSPEED.getValue() ;
					   } 
					   else
					   {
