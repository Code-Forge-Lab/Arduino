...
controls COMPONENTMINSECONDS(10,100); // Responsible for Pellet Pusher and Fan Working Time   
controls COMPONENTMAXSECONDSON(10,100); // Responsible for Pellet Pusher and Fan Working Time
...
void init_memory_defaults (bool conditiondefault = false) {
       ...
       COMPONENTMINSECONDS.setDataDefault();
...
void __PELLETPUSH () {analogWrite (PELLETPUSHERPIN, PELLETPUSHERMINSPEED.getValue());}; 
 void funPelletModeOnlyTimer ()
  ...
  printMenuFunc("Gran.Neveik.Sec",&COMPONENTMINSECONDS,"secundes:"); // nenaudoti nes nereikia
...
//  printMenuFunc("Gran.Greitis Max",&COMPONENTMINSECONDS,"secundes:");
         ...
         switch(PELLETPUSHERMODE.getValue()) 
         {
            case 1 : 
                  if (PELLETON_TIMEOUT == -1 && PELLETOFF_TIMEOUT == -1) // before ON_TIMEOUT become -1 , zero give window to step up a turn of mode enable
                             PELLETOFF_TIMEOUT =  COMPONENTMINSECONDS.getValue() * OneSec; // min = 60 seconds  + cunstom seconds    
