...
//Functions
void funTimeClient();
String transConditionalPrintBtn_group(String , String , String , String , String );
void turnOnHeatingTimeout();
void turnOnHeatingTimeoutOFF();
void Inv_On_Auto_on();
void turnOnByButtonEvent();
void one30SecTimer();
...
/*  About program
React to a high level of DC voltage (100-182v) from a Solar pannels to turn on a heater when reaching high voltages and turn off when reaching low dc voltages automatically 
*/
const static uint8_t pin_HighDC = D0;
bool sensorDoRead_HighDC = false;


const static uint8_t pin_StopLowDC = D6;
bool sensorDoRead_StopLowDC = false;
...
signed int Inv_OnOn_TotalTime = 29000;  // how mucth time shoud work a Inv_On by seconds 60sec*45min = 2700
signed int SolarOn_Sec = 0;             // turn on Inv_On by a seconds
...
void setup() {
  ...
  pinMode(pin_HighDC, INPUT);
  pinMode(pin_StopLowDC, INPUT);
...
void loop() {
        ...
        if (c == '\n') {  // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            */
            ...
            client.println(".btn-group:after { content: "
            ...
            if (sensorDoRead_HighDC)
            client.println("<p>High DC: connected");
            else 
            client.println("<p>High DC: disconnected -_-");

            if (sensorDoRead_StopLowDC)  
            client.println("<p>Low DC: connected" );
            else
            client.println("<p>Low DC: disconnected -_-" );
            ...
            client.println(transConditionalPrintBtn_group(output6State, "6", String(autoTOn1) + " on", String(autoTOn1) + " off"));
            client.println(transConditionalPrintBtn_group(output7State, "7", String(autoTOn2) + " on", String(autoTOn2) + " off"));
            client.println(transConditionalPrintBtn_group(output8State, "8", String(autoTOn3) + " on", String(autoTOn3) + " off"));
            client.println(transConditionalPrintBtn_group(output9State, "9", String(autoTOn4) + " on", String(autoTOn4) + " off"));


            client.println("<p>Auto-on:");
            if (output6State == "on")
              client.println(String(autoTOn1) + "h,");

            if (output7State == "on")
              client.println(String(autoTOn2) + "h,");

            if (output8State == "on")
              client.println(String(autoTOn3) + "h,");

            if (output9State == "on")
              client.println(String(autoTOn4) + "h");
  ...
  if (SolarOn_Sec > 0 /*|| doBtnWasPress*/) {  // if timeout still on then turn on



    if (previousState && SolarOn_Sec <= 1) {
...
void turnOnHeatingTimeout() {
  SolarOn_Sec = Inv_OnOn_TotalTime;
  powrelay_Sec = 10;
  pin_Inv_OnState = "on";
}
void turnOnHeatingTimeoutOFF() {
...
void Inv_On_Auto_on() {
  ...
  if (ActivateOneHourWhenPassed) {
    ...
    if (output6State == "on" && timeClient.getHours() == 12) {
      Serial.println(" at " + String(autoTOn1));
      turnOnHeatingTimeout();
    } else if (output7State == "on" && timeClient.getHours() == 13) {
      Serial.println(" at " + String(autoTOn2));
      turnOnHeatingTimeout();
    } else if (output8State == "on" && timeClient.getHours() == 14) {
      Serial.println(" at " + String(autoTOn3));
      turnOnHeatingTimeout();
    } else if (output9State == "on" && timeClient.getHours() == 15) {
      Serial.println(" at " + String(autoTOn4));
      turnOnHeatingTimeout();
    } else {
      ActivateOneHourWhenPassed = true;
    };  // if failed to auto-on then reset to work for a next time
...
void __main() {
  ...
  sensorDoRead_HighDC = digitalRead(pin_HighDC);        // need more then 180v dc-ac or short a test points
  sensorDoRead_StopLowDC = digitalRead(pin_StopLowDC);  // if voltage is more then 100v

  Serial.println ("High DC: " + String (sensorDoRead_HighDC) + " Low DC: "+ String ( sensorDoRead_StopLowDC));
}
...
void oneSecTimer() {
  ...
  if (clock_1secCounter >= 5) {
    ...
    if (SolarOn_Sec > 0) { SolarOn_Sec = SolarOn_Sec - 1; }     // countdown timeout
    if (powrelay_Sec > 0) { powrelay_Sec = powrelay_Sec - 1; }  /// delay to turn on a second relay
...
int getHours() {
  return ((SolarOn_Sec % 86400L) / 3600);
}
int getMinutes() {
  return ((SolarOn_Sec % 3600) / 60);
}
int getSeconds() {
  return (SolarOn_Sec % 60);
}